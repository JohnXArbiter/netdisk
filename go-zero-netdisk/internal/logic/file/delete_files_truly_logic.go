...
func (l *DeleteFilesTrulyLogic) DeleteFilesTruly(req *types.FileIdsStruct) error {
	var (
	...
	if err := engine.Cols("id").In("id", req.FileIds).And("user_id = ?", userId).
		And("del_flag = ?", constant.StatusFileDeleted).Find(&files); err != nil {
		return err
	}

	length := len(req.FileIds)
	if length != len(files) {
