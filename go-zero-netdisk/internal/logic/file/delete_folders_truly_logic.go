...
func (l *DeleteFoldersTrulyLogic) DeleteFoldersTruly(req *types.FolderIdsStruct) error {
	var (
	...
	if err := engine.Cols("id").In("id", req.FolderIds).And("user_id = ?", userId).
		And("del_flag = ?", constant.StatusFileDeleted).Find(&folders); err != nil {
		return err
	}

	length := len(req.FolderIds)
	if length != len(folders) {
		return errors.New("发生错误！")
	}

	if affected, err := engine.In("id", req.FolderIds).
