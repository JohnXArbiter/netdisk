<template>
      ...
      <div class="file-table">
        <div class="button-group">
          <template v-if="fileButtonsState !== 0">
            <el-button-group>
              <el-button type="primary" round plain :icon="Download" @click="fileButton(0)">下载</el-button>
              <template v-if="fileButtonsState === 1">
                <el-button type="primary" round plain :icon="EditPen" @click="fileButton(1)">重命名</el-button>
              </template>
              <el-button type="primary" round plain :icon="Rank" @click="fileButton(2)">移动</el-button>
              <el-button type="primary" round plain :icon="CopyDocument" @click="fileButton(3)">复制</el-button>
              <el-button type="danger" round plain :icon="DeleteFilled" @click="fileButton(4)">删除</el-button>
            </el-button-group>
          </template>
        </div>
  ...
  <el-dialog v-model="copyAndMoveDialog" title="选择文件夹">
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="copyAndMoveDialog = false">取消</el-button>
        <el-button type="primary" @click="renameFile()">
          确定
        </el-button>
      </span>
...
import {
  ...
  deleteFiles
} from '@/views/fileFolder/fileFolder.ts'
...
let copyAndMoveDialog = ref(false)
...
function fileButton(option: number) {
  ...
  if (option === 2 || option === 3) {
    toFolder(listFoldersCurrentFolderId)
    copyAndMoveDialog.value = true
    return
  }
...
async function deleteFilesConfirm() {
  const ids = selectedFile.map(file => file.id)
